...
jQuery(document).ready(function ($) {
    ...
    function fpf_init_note() {
        if (fpfCommonParams.show_messages == true) {
            ...
            var scaleHTML =
                '<div class="fpf-transform-options fpd-transform-options fpd-list-row fpd-sub-option">';
            ...
            $(".fpd-center-horizontal").append(
                '<div class="fpf-helper-btn-label">' +
                    fpfCommonParams.helperCenterHorizontal +
                    "</div>"
            );
            $(".fpd-center-vertical").css("display", "block");
            $(".fpd-center-vertical").append(
                '<div class="fpf-helper-btn-label">' +
                    fpfCommonParams.helperCenterVertical +
                    "</div>"
            );
            $(".fpd-flip-horizontal").css("display", "block");
            $(".fpd-flip-horizontal").append(
                '<div class="fpf-helper-btn-label">' +
                    fpfCommonParams.helperFlipHorizontal +
                    "</div>"
            );
            $(".fpd-flip-vertical").css("display", "block");
            $(".fpd-flip-vertical").append(
                '<div class="fpf-helper-btn-label">' +
                    fpfCommonParams.helperFlipVertical +
                    "</div>"
            );
            $(".fpd-reset-element").css("display", "block");
            $(".fpd-reset-element").append(
                '<div class="fpf-helper-btn-label">' +
                    fpfCommonParams.helperResetElement +
                    "</div>"
            );

            var action_label = fpfCommonParams.action_label;
            $(".fpd-right .fpd-icon-more").after(
                '<span class="fpf_action_label">' + action_label + "</span>"
            );
        }
    }
    ...
    function fpf_init_custom_filter() {
        ...
        fabric.Image.filters.Mask = fabric.util.createClass(
            fabric.Image.filters.BaseFilter,
            {
                ...
                initialize: function (options) {
                    ...
                    this.channel =
                        [0, 1, 2, 3].indexOf(options.channel) > -1
                            ? options.channel
                            : 0;
                },
                applyTo2d: function (options) {
                    ...
                    let maskImageData = maskCanvas
                        .getContext("2d")
                        .getImageData(0, 0, maskImage.width, maskImage.height);
        ...
        $(document).on("click", ".fpd-tool-advanced-editing", function () {
            ...
            if (!$(".fpf-filters").length) {
                ...
                var newFilterBtns =
                    "<label>" + fpfCommonParams.texture_message + "</label>";
                newFilterBtns =
                    newFilterBtns +
                    '<div class="fpd-filter-fpf0 fpf-item fpf-filters fpf-filters-btn active" data-filter="fpf0" data-executing="no">';
                newFilterBtns =
                    newFilterBtns +
                    '<img class="fpf-filter-icon" src="' +
                    fpfCommonParams.original_icon +
                    '" title="' +
                    fpfCommonParams.original_text +
                    '" alt="' +
                    fpfCommonParams.original_text +
                    '" />';
                // newFilterBtns = newFilterBtns + '<div class="fpf-clear"></div>';
                newFilterBtns =
                    newFilterBtns + '<div class="fpf-filter-icon-title">';
                ...
                $.each(textures_obj, function (texture_name, texture_values) {
                    newFilterBtns =
                        newFilterBtns +
                        '<div class="fpd-filter-fpf' +
                        btn_count +
                        ' fpf-item fpf-filters fpf-filters-btn" data-type="' +
                        texture_name +
                        '" data-filter-title="' +
                        texture_name +
                        '" data-filter="fpf' +
                ...
                let color = fancyProductDesigner.currentViewInstance.getElementByTitle(
                    "Garment Color"
                ).fill;
                if ($(".filter-style").length == 0) {
                    $("head").append(
                        '<style type="text/css" class="filter-style"></style>'
                    );
                }
                $(".filter-style").html(
                    ".fpd-image-editor-main:before {background: " + color + ";}"
                );
            }

            // Filter click action handler. Addes active class to active filter
            $(".fpd-image-editor-container").on(
                "click",
    ...
    function fpf_add_product_details() {
        ...
        // Get all the objects
        let colorTitleArray = [
            fpfCommonParams.colorLayerTitle,
            "Body Color",
            "Liner Color",
            "Cord Color",
            "Zipper Color",
            "Sleeve Color",
        ]; // All color titles used in goldengoodsusa.com
        ...
        for (let viIndex = 0; viIndex < viCount; viIndex++) {
            ...
            for (obj of viObjArray) {
                ...
                if (obj.isCustom == true) {
                    let real_width = (obj.width * obj.scaleX) / object_dpi;
                    let real_height = (obj.height * obj.scaleY) / object_dpi;
                    ...
                    filterText =
                        ...
                        " Dimensions: " +
                        " Height: " +
                        real_height.toPrecision(2) +
                        '" Width: ' +
                        real_width.toPrecision(2) +
                        '" \n ';
                    filterText =
                        filterText +
                        " Filter: " +
                        (obj.customFilters ? obj.customFilters.join(",") : "") +
                        " \n ";
    ...
    function fpf_add_watermark() {
        ...
        waterParams["x"] =
            my_stage.width -
            parseInt(w) +
            parseInt(fpfCommonParams.water_mark_left);
        waterParams["y"] =
            0 + parseInt(h) + parseInt(fpfCommonParams.water_mark_top);
    ...
    function fpf_send_quote() {
        ...
        $('[name="fpd_shortcode_form"] [name]').each(function (index, element) {
            if (typeof $(element).attr("name") != "undefined") {
                ...
                if (
                    fld_name != "fpd_shortcode_form_name" &&
                    fld_name != "fpd_shortcode_form_email" &&
                ...
                if (fld_name == "quantity_field") {
                    if (fld_value > 149) {
                        ...
                        var errorMSG =
                            "Please enter a number of 150 or greater in the quantity field";
                    }
                }
        ...
        if (validate == 4) {
            ...
            quote_form.before(
                '<div class="sending_quote"><div class="sending_quote_title">' +
                    fpfCommonParams.sending_quote_request_message +
                    '</div><img alt="Sending" src="' +
                    fpfCommonParams.sending +
                    '" /></div>'
            );
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    if (fancyProductDesigner.viewInstances[0]) {
                                        fancyProductDesigner.viewInstances[0].removeElement(
                                            "PDF Label"
                                        );
                                        // fancyProductDesigner.viewInstances[0].removeElement('Watermark');
                                    }
                                    if (fancyProductDesigner.viewInstances[1]) {
                                        fancyProductDesigner.viewInstances[1].removeElement(
                                            "PDF Label Back"
                                        );
                                        // fancyProductDesigner.viewInstances[1].removeElement(
                                        //     "Watermark"
                                    ...
                                    $(".sending_quote").html(
                                        '<div class="quote_received">' +
                                            fpfCommonParams.quote_sent_message +
                                            "</div>"
                                    ); // Show the success message
                                    $(".fpd-full-loader").removeClass(
                                        "fpf_show"
                                    ); // Remove the loader
                                    ...
                                    setTimeout(function () {
                                        ...
                                        $(
                                            '[name="fpd_shortcode_form"]'
                                        ).slideDown(); // Show form again
                                    }, 1000);
                                },
                                error: function (error) {
                                    $(".sending_quote").html(
                                        '<div class="quote_received">An error occurred - your quote was not sent.</div>'
                                    );
                                    $(".fpd-full-loader").removeClass(
                                        "fpf_show"
                                    );
            ...
            alert(
                "You did not fill out all the required information, or some information is incorrect."
            );
        }
    }
    ...
    function fpf_init_quote_form() {
        ...
        if ($(".fpd-content").length) {
            $(".fpd-content").prepend($(".print-parameters"));
            $(".print-parameters h5").click(function () {
                $(this).closest(".print-parameters").toggleClass("active");
            });
        }

        // Add step4 request quote button and size guide button
        $(".fpd-navigation").append(
            '<span class="request-quote custom-tab"><span class="fpd-nav-icon fpd-icon-share-mail"></span><span class="fpd-label">Step 4: Request a Quote</span></span>'
        );
        $(".fpd-navigation").append(
            '<div class="size-guide custom-tab" data-module="size-guide"><span class="fpd-nav-icon fpd-icon-guided-tour"></span><span class="fpd-label">Graphic Print Size Guide</span></div>'
        );

        $(".print-parameters").attr("data-module", "size-guide");

        // Request quote button click handler
        $(".fpd-product-designer-wrapper").on(
            "click",
            ".request-quote",
            function () {
        ...
        /*// Request size-guide button click handler
        $(".fpd-product-designer-wrapper").on(
            "click",
            ".size-guide",
            function () {
                // Remove active from the other tabs
                $(this).siblings("div").removeClass("fpd-active");
                $(".fpd-content>div").removeClass("fpd-active");

                // Set active for current tab and show the size guid info
                $(this).addClass("fpd-active");
                $(".fpd-content .print-parameters").addClass("fpd-active");
            }
        );*/
        ...
        $('[name="fpd_shortcode_form_name"]').before(
            "<label>" +
                fpfCommonParams.project_title_placeholder +
                '</label><input type="text" name="project_title" class="fpd-shortcode-form-text-input project_title" />'
        );

        // Add label to name field and hide placeholder
        $('[name="fpd_shortcode_form_name"]').before(
            "<label>" +
                fpfCommonParams.name_placeholder +
                '<span class="required">*</span></label>'
        );
        $('[name="fpd_shortcode_form_name"]').attr("placeholder", "");

        // Add label to email field and hide placeholder
        $('[name="fpd_shortcode_form_email"]').before(
            "<label>" +
                fpfCommonParams.email_placeholder +
                '<span class="required">*</span></label>'
        );
        ...
        let fieldGroup =
            "<label>" +
            fpfCommonParams.quantity_placeholder +
            '<span class="required">*</span></label><input type="text" name="quantity_field" class="fpd-shortcode-form-text-input quantity_field" />';
        fieldGroup +=
            '<label>Promo code (optional)</label><input type="text" name="code_field" class="fpd-shortcode-form-text-input code_field" />';
        fieldGroup +=
            "<label>" +
            fpfCommonParams.date_needed_by_placeholder +
            '<span class="required">*</span></label><input type="date" name="date_need_by" class="fpd-shortcode-form-text-input date_need_by"" />';
        fieldGroup +=
            '<span class="under_quote_message">' +
            fpfCommonParams.under_quote_message +
            "</span>";
        fieldGroup +=
            "<label>" +
            fpfCommonParams.comments_placeholder +
            '</label><textarea name="comment_field" class="fpd-shortcode-form-text-input comment_field" />';
        fieldGroup +=
            '<input type="button" value="Submit" class="fpf-submit-btn button">';
        fieldGroup +=
            '<div class="bottom_quote_message">' +
            fpfCommonParams.bottom_quote_message +
            '<br><br><p>Need help? email us at <a href="mailto:info@goldengoodsusa.com">info@goldengoodsusa.com</a></p></div>';
