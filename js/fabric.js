    ...
    function() {
        ...
        fabric.StaticCanvas = fabric.util.createClass({
            ...
            _createLowerCanvas: function(e) {
                this.lowerCanvasEl = fabric.util.getById(e) || this._createCanvasElement(), this._initCanvasElement(this.lowerCanvasEl), fabric.util.addClass(this.lowerCanvasEl, "lower-canvas").setAttribute("crossOrigin", "Anonymous");, this.interactive && this._applyCanvasStyle(this.lowerCanvasEl), this.contextContainer = this.lowerCanvasEl.getContext("2d")
            },
            getWidth: function() {
                return this.width
            },
        ...
        fabric.util.object.extend(fabric.Object.prototype, {
            ...
            setCoords: function() {
                var t = this.strokeWidth,
                    ...
                    s = this.width,
                    o, u = this.height,
                    a, f = this.strokeLineCap === "round" || this.strokeLineCap === "square",
                    l = this.type === "line" && this.width === 0,
                    c = this.type === "line" && this.height === 0,
                    h = l || c,
                    p = f && c || !h,
                    d = f && l || !h;
                l ? s = t : c && (u = t), p && (s += s > 0 ? t : -t), d && (u += u > 0 ? t : -t), o = s * this.scaleX + 2 * this.padding, a = u * this.scaleY + 2 * this.padding, o < 0 && (o = Math.abs(o));
                var v = Math.sqrt(Math.pow(o / 2, 2) + Math.pow(a / 2, 2)),
                    m = Math.atan(isFinite(a / o) ? a / o : 0),
                    g = Math.cos(m + n) * v,
                    y = Math.sin(m + n) * v,
                    b = Math.sin(n),
                    w = Math.cos(n),
                    E = this.getCenterPoint(),
                    S = new fabric.Point(o, a),
                    x = new fabric.Point(E.x - g, E.y - y),
                    T = new fabric.Point(x.x + S.x * w, x.y + S.x * b),
                    N = new fabric.Point(x.x - S.y * b, x.y + S.y * w),
                    C = new fabric.Point(x.x + S.x / 2 * w, x.y + S.x / 2 * b),
                    k = i(x),
                    L = i(T),
                    A = i(new fabric.Point(T.x - S.y * b, T.y + S.y * w)),
                    O = i(N),
                    M = i(new fabric.Point(x.x - S.y / 2 * b, x.y + S.y / 2 * w)),
                    _ = i(C),
                    D = i(new fabric.Point(T.x - S.y / 2 * b, T.y + S.y / 2 * w)),
                    P = i(new fabric.Point(N.x + S.x / 2 * w, N.y + S.x / 2 * b)),
                    H = i(new fabric.Point(C.x, C.y));
                return this.oCoords = {
                    tl: k,
                    tr: L,
                    br: A,
